...
namespace DotnetAPI.Controllers {
    ...
    public class PostController : ControllerBase {
        ...
        public IEnumerable<Post> GetPosts(int postId = 0, int userId = 0, string searchParam = "None") {
            ...
            string stringParamaters = "";

            DynamicParameters sqlParameters = new DynamicParameters();
            if(postId != 0) {
                stringParamaters += ", @PostId=" + postId.ToString();
                sqlParameters.Add("@PostIdParameter", postId, DbType.Int32);
            }
            if(userId != 0) {
                stringParamaters += ", @UserId=" + userId.ToString();
                sqlParameters.Add("@UserIdParameter", postId, DbType.Int32);
            }
             if(searchParam.ToLower() != "none") {
                stringParamaters += ", @SearchValue='" + searchParam + "'";
                sqlParameters.Add("@SearchValueParameters", searchParam, DbType.String);
            }

            if(stringParamaters.Length > 0) {
                sql += stringParamaters.Substring(1);
            }
         ...
         [HttpGet("MyPosts")]
        public IEnumerable<Post> GetPostMyPosts() {
        ...
        public IActionResult UpsertPost(Post postToUpsert) {
            ...
            throw new Exception("Failed to upsert the post!");
        }
